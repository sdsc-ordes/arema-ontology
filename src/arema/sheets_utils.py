...
def get_sheet_last_modified(key_file: str | None = None, sheet_id: str | None = None) -> datetime:
    """
    ...
    """
    key_file = key_file or os.environ.get("SERVICE_ACCOUNT_KEY", "/app/service_account.json")
    sheet_id = sheet_id or os.environ.get("GOOGLE_SHEET_ID", "1RL6Y120_H9-yD8x52eZO44S2iLQpLoZHitcExHsPfPs")
...
def has_sheet_been_modified_since(since: datetime, key_file: str | None = None, sheet_id: str | None = None) -> bool:
    """
    ...
    try:
        ...
        if is_modified:
            logger.info(f"Sheet modified at {last_modified.isoformat()}, after {since.isoformat()}")
        else:
            logger.info(f"Sheet not modified since {since.isoformat()} (last modified: {last_modified.isoformat()})")
        
        return is_modified
        
    except Exception as e:
        logger.warning(f"Could not check sheet modification time: {str(e)}. Assuming sheet was modified.")
        # If we can't check, assume it was modified to be safe
        return True
