...
def run_conversion():
    ...
    is_valid = shacl.run_shacl_validation(str(csv2ont.TTL_PATH), str(SHAPES_FILE))

    if not is_valid:
        logger.error(
            "Ontology validation failed: graph does not conform to SHACL shapes"
        )
    ...
    if create_github_release():
        logger.info(
            "✅ GitHub release created - this will trigger documentation generation"
        )
    else:
